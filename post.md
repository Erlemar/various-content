...
На вход подаются обработанные изображения, далее идёт скрытый слой, в котором 100 нейронов. В качестве функции активации скрытого слоя используется ReLU. На выходе имеем 10 нейронов с активацией softmax. Softmax даёт вероятность принадлежности к тому или иному классу. Разберём код для FNN подробнее.

Вначале необходимо инициализировать веса. Биасы изначально нулевые, а вот для самих весов я решил использовать инициализацию Ксавьера. При таком варианте инициализации дисперсия берётся равной 2 / количество входящих нейронов. Благодаря этому веса получаются не слишком большими и не слишком маленькими.

Для удобства шаг градиентного спуска определяется отдельным методом
