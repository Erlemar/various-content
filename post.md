...
* В методе loss реализован шаг градиентного спуска и расчёт loss с учетом L2 регуляризации;
* В методе train осуществляется итеративная тренировка на мини-батчах. Loss считается после каждого шага. После каждой эпохи считается точность на тренировочном и валидационном датасетах, а также уменьшается learning rate;
...
* количество итераций и размер мини-батча, их частное - это количество эпох;
...
Думаю, что сразу заметно, что точность на MNIST довольно низка. Я было забеспокоился, но подумал о том, что наверняка собранные мной данные и данные MNIST могут различаться. Чтобы проверить это я взял CNN из официального [гайда](https://www.tensorflow.org/get_started/mnist/pros) Tensorflow для MNIST и натренировал её. На тренировочной части MNIST точность была 99%+ (как и ожидалось), а вот на моих данных точность оказалась значительно ниже **тут точность**. Исходя из этого я решил, что моя модель достаточно хороша и не стоит пытаться сделать её лучше.
...
* Входной изображение обрабатывается, аугментируется до 24 и модель делает один шаг градиентного спуска на этих данных;
...
Выбор learning rate был самым сложным. Изначальное значение было 0.1, а после 24 эпох 0.1 * (0.95 ^ 24). Проблема в том, что это значение использовалось для всей эпохи - 800 итераций. Если использовать это значение, то каждое новое изображение может значительно изменять веса. Я пробовал много разных значений и остановился на 0.1 * (0.95 ^ 24) / 32. В итоге одно изображение не сможет значительно изменить веса, но несколько изображений постепенно приведут к заметным изменениям.
